...
public class BattleScene {
    ...
    private Boolean makeHit(FantasyCharacter defender, FantasyCharacter attacker, Realm.FightCallback fightCallback){
        ...
        int fat = (int) attacker.fatal();
        int defenderHealth = defender.getHealthPoints() -hit;
        int FatakCrit = defender.getHealthPoints()-fat;

        if (fat >= 10){
            System.out.println(String.format("%s нанес критический удар в %d единиц!", attacker.getName(), fat));
            System.out.println(String.format("У %s осталось %d единиц здоровья...", defender.getName(),FatakCrit));
        }
